        ...
        @foreach ($reservas as $reserva)
            <thead>
                <tr>
                    ...
                    <td>{{$reserva->codigodopatrimonio}}</td>
                    <td>{{$reserva->datainicioreserva}}</td>
                    <td>{{$reserva->datafimreserva}}</td>                                                                 
                    <td>{{$reserva->quantidadeitensreservados}}</td>                                                                 
                    <td><form action="{{route('reservas.edit', ['id' => $reserva->id])}}" method="get">
                        @csrf
                        <input type="submit" class="btn btn-primary" name="formulario" value="Alterar">
                        </form></td>

                    <td><form action="{{route ('reservas.delete', ['id' => $reserva->id])}}" method="POST">
                        @csrf
                        @method('DELETE')   
