...
<div class="container-fluid">
    ...
    <div class="ms-5 me-5 mt-1 mb-1 container-conteudo bg-light p-4">
        ...
        <div class="row">
            <form action="{{ route('reservas.indexBuscar') }}" class="d-flex justify-content-around w-auto" method="GET">
                @csrf
                <div class="row">
                    <div style="display: flex;">
                        ...
                        <a href="{{ route('reservas.index') }}" class="btn btn-success m-2">Limpar Filtro</a>
                    </div>
                </div>
            </form>
        </div>
            ...
            <table class="table cabecalho-itens text-center p-2" id="conteudo-itens-lado-direito">
                ...
                <tbody class="conteudo-itens">
                    <tr>
                        ...
                        <td>
                            @if(Auth::user()->access_level === 'admin' || (Auth::user()->access_level === 'user' &&  Auth::user()->id === $reserva->usuario_id))
                        ...
                        <td>
                            @if(Auth::user()->access_level === 'admin' || (Auth::user()->access_level === 'user' &&  Auth::user()->id === $reserva->usuario_id))
                            <div class="col" id="meio">
                                <form id="formExcluir" action="{{route('reservas.delete', ['id' => $reserva->id])}}" method="POST">
                                    @csrf
                                    @method('DELETE')
