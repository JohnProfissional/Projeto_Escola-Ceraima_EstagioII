...
<div class="container-fluid">
    ...
    <div class="ms-5 me-5 mt-1 mb-1 container-conteudo bg-light p-4">
        ...
        <div class="row">
            <form action="{{ route('bensexcedentes.indexBuscar') }}" class="d-flex justify-content-around w-auto" method="GET">
                @csrf
                <div class="row">
                    <div style="display: flex;">
                        ...
                        <a href="{{ route('bensexcedentes.index') }}"  class="btn btn-success m-2">Limpar Filtro</a>
                    </div>
                </div>
            </form>
        </div>
        ...
        <table class="table cabecalho-itens text-center p-2" id="conteudo-itens-lado-direito">
            ...
            <tbody class="conteudo-itens"> <!--class="row conteudo-itens w-auto h-auto p-2" id="conteudo-itens-lado-direito">-->
                <tr>
                    ...
                    <td>{{$bemexcedente->descricao}}</td>
                    <td>{{$bemexcedente->observacoes}}</td>
                    <td>{{$bemexcedente->quantidadeexcedente}}</td>
                    <td>{{$bemexcedente->acessarComodo->descricaodocomodo}}</td>

                    @can('access')
                    <td>
                        <div class="col" id="meio">
                            <form action="{{route('bensexcedentes.edit', ['id' => $bemexcedente->id])}}" method="get">
                                @csrf
                    ...
                    <td>
                        <div class="col" id="meio">
                            <form id="formExcluir" action="{{route('bensexcedentes.delete', ['id' => $bemexcedente->id])}}" method="POST">
                                @csrf
                                @method('DELETE')
