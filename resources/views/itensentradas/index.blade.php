...
<head>
	...
	<title>Lista de Itens Entradas</title>
</head>
<body>
	<div id="header">
		<h1>@yield('titulo')</h1>
    ...
    <div class="card-body">
                ...
                @endif
    @section('titulo','Lista de Itens Entradas')

    <h1>Itens Entradas</h1>
        ...
        @foreach ($itensentradas as $itensentrada)
            <thead>
                <tr>
                    ...
                    <td scope="row">{{$itensentrada->id}}</td>
                    <td>{{$itensentrada->codbem}}</td>                                             
                    <td>{{$itensentrada->descricaodobem}}</td>                                      
                    <td>{{$itensentrada->tombo}}</td>
                    <td>{{$itensentrada->valordobem}}</td>
                    <td>{{$itensentrada->historicodatransferencia}}</td>
                    <td>{{$itensentrada->numerodanotafiscal}}</td>                                             
                    <td>{{$itensentrada->datadanotafiscal}}</td>                                             
                    <td>{{$itensentrada->quantidade}}</td>                                             
                    <td><form action="{{route('itensentradas.edit', ['id' => $itensentrada->id])}}" method="get">
                        @csrf
                        <input type="submit" class="btn btn-primary" name="formulario" value="Alterar">
                        </form></td>

                    <td><form action="{{route ('itensentradas.delete', ['id' => $itensentrada->id])}}" method="POST">
                        @csrf
                        @method('DELETE')   
    ...
    <form action="{{route('itensentradas.create')}}" method="get">
        @csrf        
