...
@section('title', 'Patrimônios Cedidos')
...
<div class="container-fluid">
    ...
    <div class="ms-5 me-5 mt-1 mb-1 container-conteudo bg-light p-4">
        ...
        <div class="row">
            <form action="{{ route('cedidos.indexBuscar') }}" class="d-flex justify-content-around w-auto" method="GET">
                @csrf
                <div class="row">
                    <div style="display: flex;">
                        <input type="text" name="searchInstituicao" class="w-auto m-2 form-control" id="campoDeBusca" placeholder="Pesquisar Instituição Receptora...">
                        ...
                        <a href="{{ route('cedidos.index') }}"  class="btn btn-success m-2">Limpar Filtro</a>
                    </div>
                </div>
            </form>
        </div>
        ...
        <div class="row m-3">
            <table class="table cabecalho-itens text-center p-2" id="conteudo-itens-lado-direito">
                ...
                <tbody class="conteudo-itens"> <!--class="row conteudo-itens w-auto h-auto p-2" id="conteudo-itens-lado-direito">-->
                    <tr>
                        <td scope="row">{{$cedido->id}}</td>
                        <td>{{$cedido->instituicaoreceptora}}</td>
                        <td>{{ \Carbon\Carbon::parse($cedido->datacedido)->format('d/m/Y')}}</td>
                        <td>{{$cedido->acessarPatrimonio->descricaodopatrimonio}}</td>
                        <td>{{$cedido->qtd}}</td>

                        @can('access')
                        <td>
                            <div class="col" id="meio">
                                <form action="{{route('cedidos.edit', ['id' => $cedido->id])}}" method="get">
                                    @csrf
                        ...
                        <td>
                            <div class="col" id="meio">
                                <form id="formExcluir" action="{{route('cedidos.delete', ['id' => $cedido->id])}}" method="POST">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="col-lg-12 me-3" style="text-align:right">
            <form action="{{route('cedidos.create')}}" method="get">
                @csrf
