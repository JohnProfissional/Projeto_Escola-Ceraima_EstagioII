...
<body>
        ...
        @foreach ($saidas as $saida)
            <thead>
                <tr>
                    ...
                    <td scope="row">{{$saida->id}}</td>
                    <td>{{$saida->datasaida}}</td>                                             
                    <td>{{$saida->statussaida}}</td> 
                    <td>{{$saida->quantidadetotalsaida}}</td> 
                    <td><form action="{{route('saidas.edit', ['id' => $saida->id])}}" method="get">
                        @csrf
                        <input type="submit" class="btn btn-primary" name="formulario" value="Alterar">
                        </form></td>

                    <td><form action="{{route ('saidas.delete', ['id' => $saida->id])}}" method="POST">
                        @csrf
                        @method('DELETE')   
    ...
    <form action="{{route('saidas.create')}}" method="get">
        @csrf        
