...
@section('titulo','Lista de Empréstimos')
  ...
  <table class="table table-sm">
      ...
      <thead>
        <tr>
          <th scope="col">id</th>
          <th>nomedobememprestado</th><br>
          <th>coddobem</th>
          <th>nomedoprofissional</th>
          <th>quantidadeemprestada</th>
          <th>saida_id</th>
                    
        </tr>
      </thead>
       
            <td scope="row">{{$emprestimos->id}}</td>

          <td>{{$emprestimos->nome}}</td>
          
             <td>
                   <form action="{{route('emprestimos.delete', ['id' => $emprestimos->id])}}" method="post">
                    @csrf
                    @method('DELETE')   
                ...
                <td>
                    <form action="{{route('emprestimos.edit', ['id' => $emprestimos->id])}}" method="post">
                        @csrf
     ...
     <form action="{{route('emprestimos.create')}}" method="post">
                        @csrf
                        <input type="submit" class="btn btn-primary" name="formulario" value="cadastrar novo empréstimo">
                    </form>
