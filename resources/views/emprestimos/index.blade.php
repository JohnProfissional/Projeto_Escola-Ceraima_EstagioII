        ...
        @foreach ($emprestimos as $emprestimo)
            <thead>
                <tr>
                    ...
                    <th>Código do Patrimônio</th>                                                            
                    <th>Nome do Funcionário</th>
                    <th>Quantidade</th>
                </tr>
            </thead>       
                    <td scope="row">{{$emprestimo->id}}</td>
                    <td>{{$emprestimo->nomedobememprestado}}</td>                                             
                    <td>{{$emprestimo->coddobem}}</td>                                      
                    <td>{{$emprestimo->nomedoprofissional}}</td>
                    <td>{{$emprestimo->quantidadeemprestada}}</td>
                    <td><form action="{{route('emprestimos.edit', ['id' => $emprestimo->id])}}" method="get">
                        @csrf
                        <input type="submit" class="btn btn-primary" name="formulario" value="Alterar">
                        </form></td>

                    <td><form action="{{route ('emprestimos.delete', ['id' => $emprestimo->id])}}" method="POST">
                        @csrf
                        @method('DELETE')   
