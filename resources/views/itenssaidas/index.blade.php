...
<head>
	...
	<title>Lista de Itens Saídas</title>
</head>
<body>
	<div id="header">
		<h1>@yield('titulo')</h1>
    ...
    <div class="card-body">
                ...
                @endif
    @section('titulo','Lista de Itens Saídas')

    <h1>Itens Saídas</h1>

    <table class="table table-sm">   

        @foreach ($itenssaidas as $itenssaida)
            <thead>
                <tr>
                    <th scope="col">Id</th>
                    <th>Descrição</th>
                    ...
                    <td scope="row">{{$itenssaida->id}}</td>
                    <td>{{$itenssaida->descricaodopatrimonio}}</td>                                             
                    <td>{{$itenssaida->codigopatrimonio}}</td> 
                    <td>{{$itenssaida->quantidadesaida}}</td>   
                    <td><form action="{{route('itenssaidas.edit', ['id' => $itenssaida->id])}}" method="get">
                        @csrf
                        <input type="submit" class="btn btn-primary" name="formulario" value="Alterar">
                        </form></td>

                    <td><form action="{{route ('itenssaidas.delete', ['id' => $itenssaida->id])}}" method="POST">
                        @csrf
                        @method('DELETE')   
    ...
    <form action="{{route('itenssaidas.create')}}" method="get">
        @csrf        
